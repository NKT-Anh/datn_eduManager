...
exports.updateClass = async (req, res) => {
  try {
    ...
    // ✅ Nếu thay đổi GVCN (có GVCN cũ), gỡ lớp khỏi homeroomClassIds của GVCN cũ
    // ✅ Lưu ý: KHÔNG xóa khỏi homeroomClassIds vì đó là lịch sử, chỉ reset currentHomeroomClassId
    if (oldClass && oldClass.teacherId && oldClass.teacherId.toString() !== teacherId) {
      ...
      // ✅ Kiểm tra xem GVCN cũ còn lớp chủ nhiệm nào trong năm học hiện tại không
      const oldTeacher = await Teacher.findById(oldClass.teacherId);
      if (oldTeacher) {
        // Kiểm tra xem GVCN cũ còn lớp chủ nhiệm nào trong năm học hiện tại
        const currentYearClasses = await Class.find({
          teacherId: oldClass.teacherId,
          year: currentSchoolYear
        });
        
        // Nếu không còn lớp chủ nhiệm nào trong năm học hiện tại, có thể set isHomeroom = false
        // Nhưng vẫn giữ lại nếu có lớp chủ nhiệm ở các năm học khác
        if (currentYearClasses.length === 0 && (!oldTeacher.homeroomClassIds || oldTeacher.homeroomClassIds.length === 0)) {
          await Teacher.findByIdAndUpdate(oldClass.teacherId, {
            isHomeroom: false
          });
        }
...
exports.deleteClass = async (req, res) => {
  ...
  try {
    ...
    if (cls.teacherId) {
      ...
      // ✅ Kiểm tra xem GVCN còn lớp chủ nhiệm nào trong năm học hiện tại không
      ...
      if (teacher) {
        ...
        if (currentYearClasses.length === 0 && (!teacher.homeroomClassIds || teacher.homeroomClassIds.length === 0)) {
          await Teacher.findByIdAndUpdate(cls.teacherId, {
            isHomeroom: false
          });
        }
