...
const teacherSchema = new mongoose.Schema({
  ...
  // ==============================
  // Lưu role/chức vụ theo từng năm học mà KHÔNG xóa các trường boolean hiện tại.
  yearRoles: [{
    schoolYear: { type: String, required: true }, // ví dụ: "2025-2026"
    // Lưu tổ bộ môn / department theo từng năm học
    departmentId: { type: mongoose.Schema.Types.ObjectId, ref: 'Department', default: null },
    isHomeroom: { type: Boolean, default: false }, // Giáo viên chủ nhiệm
    isDepartmentHead: { type: Boolean, default: false }, // Trưởng bộ môn
    // ✅ isLeader đã được loại bỏ - BGH được set cứng ở top-level (teacher.isLeader) để luôn truy cập ở mọi năm
    permissions: { type: [String], default: [] }, // Quyền bổ sung
    currentHomeroomClassId: { type: mongoose.Schema.Types.ObjectId, ref: 'Class', default: null } // Lớp chủ nhiệm hiện tại
  }],
...
// Index để query nhanh theo năm học trong yearRoles
teacherSchema.index({ 'yearRoles.schoolYear': 1 });
 ...
 */
/**
 * Lấy role theo năm học (nếu có)
 * @param {String} schoolYear ví dụ: "2025-2026"
 * @returns {Object|null} entry trong `yearRoles` hoặc null nếu không tìm thấy
 */
teacherSchema.methods.getRoleForYear = function(schoolYear) {
  if (!schoolYear) return null;
  const roles = this.yearRoles || [];
  // Tìm exact match trước
  const found = roles.find(r => String(r.schoolYear) === String(schoolYear));
  return found || null;
};

teacherSchema.methods.getEffectiveWeeklyLessons = function(options = {}) {
  const { subject } = options;
  // Support specifying a school year to use year-specific roles
  const schoolYear = options.year || options.schoolYear || null;
  // If a schoolYear is provided, consult yearRoles for role flags
  const roleForYear = schoolYear ? this.getRoleForYear(schoolYear) : null;
  ...
  // Determine role flags (prefer year-specific role when available)
  const isHomeroomFlag = roleForYear ? Boolean(roleForYear.isHomeroom) : Boolean(this.isHomeroom || this.currentHomeroomClassId);
  const isDepartmentHeadFlag = roleForYear ? Boolean(roleForYear.isDepartmentHead) : Boolean(this.isDepartmentHead);
