 ...
 * @deprecated Sử dụng getEffectivePermissions và check trong array thay vì function này
 */
export const hasPermission = (role: string, permission: string): boolean => {
  // Admin có tất cả quyền
  if (role === ROLES.ADMIN) return true;
  
  // BGH (teacher với isLeader) có tất cả quyền - cần check trong usePermissions hook
...
/**
 * Mapping permissions theo role (tương tự backend)
 */
const ROLE_PERMISSIONS: Record<string, string[]> = {
  ...
  [ROLES.TEACHER]: [
    ...
    PERMISSIONS.EXAM_GRADE_ENTER,
    PERMISSIONS.EXAM_PRINT_TICKET_TEACHING,
    PERMISSIONS.DASHBOARD_VIEW_TEACHING,
 ...
 * - Admin: có tất cả quyền
 * - BGH (isLeader): có tất cả quyền
 * - Teacher: quyền cơ bản + quyền từ teacherFlags.permissions + quyền từ flags (isHomeroom, isDepartmentHead)
 * - Student: quyền cơ bản
 */
export const getEffectivePermissions = (user: BackendUser | null): string[] => {
  if (!user) return [];
  ...
  // BGH (teacher với isLeader) có tất cả quyền
  if (role === ROLES.TEACHER && user.teacherFlags?.isLeader === true) {
    return Object.values(PERMISSIONS);
  }

  // Lấy quyền cơ bản theo role
  const basePermissions = ROLE_PERMISSIONS[role] || [];

  // Nếu là teacher, thêm quyền từ teacherFlags.permissions
  let additionalPermissions: string[] = [];
  if (role === ROLES.TEACHER && user.teacherFlags?.permissions) {
    const customPermissions = Array.isArray(user.teacherFlags.permissions)
      ? user.teacherFlags.permissions
      : [user.teacherFlags.permissions];
    additionalPermissions = customPermissions.filter((p): p is string => typeof p === 'string');
  }

  // Nếu là teacher với flags đặc biệt, thêm quyền tương ứng
  if (role === ROLES.TEACHER) {
    const flags = user.teacherFlags || {};
    
    // GVCN (isHomeroom) có thêm quyền quản lý lớp chủ nhiệm
    if (flags.isHomeroom) {
    ...
    // QLBM (isDepartmentHead) có thêm quyền quản lý tổ bộ môn
