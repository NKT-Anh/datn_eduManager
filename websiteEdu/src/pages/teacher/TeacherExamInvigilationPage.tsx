...
export default function TeacherExamInvigilationPage() {
  const { backendUser } = useAuth();
  const [exams, setExams] = useState<Exam[]>([]);
  const [examId, setExamId] = useState<string>('');
  const [schedules, setSchedules] = useState<any[]>([]);
  const teacherId = backendUser?.teacherId || backendUser?.userId || backendUser?.id;
  ...
  useEffect(() => {
    if (!teacherId) return;
    examApi.getTeacherSchedules(String(teacherId), examId || undefined)
        ...
        <table className="min-w-full text-sm border">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th className="border px-3 py-2 text-left">Lớp</th>
              <th className="border px-3 py-2 text-left">Sĩ số</th>
            </tr>
          </thead>
          <tbody>
            {schedules.map((s, idx) => (
              <tr key={idx}>
                ...
                <td className="border px-3 py-2">{s.studentsInRoom}</td>
              </tr>
            ))}
            {schedules.length === 0 && (
              <tr>
                <td className="border px-3 py-6 text-center text-gray-500" colSpan={6}>Không có lịch</td>
              </tr>
            )}
          </tbody>
        </table>
