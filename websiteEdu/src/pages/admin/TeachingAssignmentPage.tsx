...
import { X, BookOpen, Loader2, AlertTriangle, Search, FileText, ChevronDown, ChevronUp } from "lucide-react";
...
export default function TeachingAssignmentPage() {
  ...
  // ✅ Load teacher status (current/effective/remaining lessons)
  useEffect(() => {
    ...
    fetchTeacherLoad();
  }, [filterYear, filterSemester, assignments.length]);
                                          ...
                                          <SelectTrigger className={`w-full h-9 ${hasTeacher ? "border-primary/50 bg-primary/5 hover:bg-primary/10" : "border-orange-200 dark:border-orange-800 hover:bg-orange-100/50 dark:hover:bg-orange-900/20"} transition-colors`}>
                                            <SelectValue placeholder="Chọn giáo viên">
                                              {cellAssignment?.teacherId?.name ? (
                                                <>
                                                  {cellAssignment.teacherId.name}
                                                  {teacherLoadMap[cellAssignment.teacherId._id]?.current !== undefined && (
                                                    <span className="ml-2 text-xs text-muted-foreground">
                                                      {teacherLoadMap[cellAssignment.teacherId._id].current}/
                                                      {teacherLoadMap[cellAssignment.teacherId._id].effective} tiết
                                                    </span>
                                                  )}
                                                </>
                                              ) : (
    ...
    <SelectContent>
                                            ...
                                            {availableTeachersForSubject.length > 0 ? (
                                              availableTeachersForSubject.map(teacher => {
                                                const loadInfo = teacherLoadMap[teacher._id] || {
                                                  current: 0,
                                                  effective: teacher.effectiveWeeklyLessons || teacher.weeklyLessons || 17,
                                                  remaining: (teacher.effectiveWeeklyLessons || teacher.weeklyLessons || 17),
                                                };
                                                const remaining = Math.max(0, loadInfo.effective - loadInfo.current);
                                                const isOver = loadInfo.current >= loadInfo.effective;
                                                return (
                                                    ...
                                                    <div className="flex flex-col space-y-1">
                                                      <span className="font-medium flex items-center justify-between gap-2">
                                                        ...
                                                        <span className={`text-xs font-semibold ${isOver ? "text-destructive" : "text-emerald-600"}`}>
                                                          {loadInfo.current}/{loadInfo.effective} tiết
                                                        </span>
                                                      </span>
