...
const TeachersList = () => {
  ...
  // ✅ Sử dụng hooks
  // State for teachers (override for QLBM)
  const {
  ...
  // If QLBM, only show teachers in their department
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (currentSchoolYear && teachers.length > 0 && classes.length > 0) {
  ...
  }, [currentSchoolYear, teachers.length, classes.length, assignments.length]);

  // ✅ Nhóm assignments theo teacherId cho năm học hiện tại
  const teachingClassesByTeacher = useMemo(() => {
    if (!currentSchoolYear || !assignments.length) return new Map<string, Set<string>>();
    
    const map = new Map<string, Set<string>>();
    
    assignments
      .filter(a => a.year === currentSchoolYear)
      .forEach(assignment => {
  ...
  const filteredTeachers = useMemo(() => {
    ...
    return teachers.filter((t) => {
      // ✅ Loại bỏ giáo viên BGH (Ban giám hiệu) khỏi danh sách
      ...
      const matchesSearch = !searchTerm || 
        ...
        // Tìm theo tổ bộ môn
        (typeof t.departmentId === 'object' && t.departmentId !== null && t.departmentId?.name?.toLowerCase().includes(lowerSearch)) ||
        // Tìm theo flags
        (t.isHomeroom && 'gvcn'.includes(lowerSearch)) ||
        (t.isDepartmentHead && 'tbm trưởng bộ môn'.includes(lowerSearch)) ||
        (t.isLeader && 'bgh ban giám hiệu'.includes(lowerSearch));

      // 🎯 Lọc theo tổ bộ môn
      const matchesDepartment = filterDepartment === 'all' || 
        (filterDepartment === 'none' && !t.departmentId) ||
        (filterDepartment !== 'all' && filterDepartment !== 'none' && t.departmentId &&
          (typeof t.departmentId === 'object' && t.departmentId !== null
            ? t.departmentId._id === filterDepartment
            : t.departmentId === filterDepartment));
      ...
      const matchesFlag = filterFlag === 'all' ||
        (filterFlag === 'homeroom' && t.isHomeroom) ||
        (filterFlag === 'departmentHead' && t.isDepartmentHead) ||
        (filterFlag === 'normal' && !t.isHomeroom && !t.isDepartmentHead);
  ...
  const handleSuggestTeachers = async (subjectId: string, subjectName: string) => {
    ...
    try {
      ...
      const teachersForSubject = teachers.filter(teacher => {
        ...
        if (teacher.isLeader) return false;
  ...
  // ✅ Lọc giáo viên không phải BGH để tính thống kê
  const nonBghTeachers = useMemo(() => {
    return teachers.filter(t => !t.isLeader);
  }, [teachers]);
                  ...
                  {missingTeachersData && !checkMissingLoading && (
                    <div className="mt-2 space-y-2">
                      <p className="text-sm text-muted-foreground">
                        Năm học {currentSchoolYear} - Học kỳ 1 | 
            ...
            <Table className="w-full table-auto">
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Tổ BM</TableHead>
                  <TableHead>Môn chính</TableHead>
                  <TableHead>Các môn dạy</TableHead>
              ...
              {filteredTeachers.length === 0 ? (
                ...
                filteredTeachers.map((teacher) => {
                  const departmentName = typeof teacher.departmentId === 'object' && teacher.departmentId?.name
                  ...
                  const teacherAssignments = assignments.filter(
                    (a) =>
                      ...
                      (!currentSchoolYear || a.year === currentSchoolYear) &&
                      a.semester === "1"
                  );
                  ...
                  // ✅ Sử dụng effectiveWeeklyLessons từ backend (virtual field)
                  ...
                  return (
                    ...
                    <TableCell>
                        {departmentName !== '-' ? (
                          <Badge variant="secondary" className="text-xs">{departmentName}</Badge>
                        ) : (
                          ...
                          <Badge variant="default" className="text-xs">
                            {typeof teacher.mainSubject === 'object' 
                              ...
                              : (() => {
                                  ...
                                  if (!mainSubjectId) return "-";
                                  return subjects.find(s => s._id === mainSubjectId)?.name || '-';
                                })()}
                          </Badge>
                        ) : (
                          <span className="text-muted-foreground text-sm">-</span>
                        ...
                        {teacher.subjects && teacher.subjects.length > 0 ? (
                          teacher.subjects.map((sub) => {
                            ...
                            const subjectName = typeof sub.subjectId === 'object' && sub.subjectId?.name
                              ? sub.subjectId.name
                              : subjects.find(s => s._id === subjectId)?.name || 'Chưa có tên';
                            const key = typeof subjectId === 'string' ? subjectId : (subjectId?._id || Math.random().toString());
                            return (
