...
interface RequiredSubject {
  ...
  classificationType?: 'excellent' | 'good' | 'average' | 'weak'; // Loại xếp loại áp dụng điều kiện này
}
...
const DEFAULT_REQUIRED_SUBJECTS: Omit<RequiredSubject, 'subjectId'>[] = [
  {
    ...
    classificationType: 'excellent', // Mặc định cho loại Giỏi
  },
  {
    minScore: DEFAULT_REQUIRED_MIN_SCORE,
    groupId: 'group_math_lit',
...
// ✅ Helper để format điểm số (luôn dùng dấu .) - chỉ format khi hiển thị, không format khi đang nhập
const formatScore = (value: number | string | undefined | null): string => {
  if (value === undefined || value === null || value === '') return '';
  if (typeof value === 'string') {
    // Nếu là string rỗng hoặc chỉ có khoảng trắng
    if (value.trim() === '') return '';
    // Thay dấu , thành dấu .
    const normalized = value.replace(',', '.');
    const num = parseFloat(normalized);
    if (isNaN(num)) return '';
    // Format về 1 chữ số thập phân (8 -> 8.0) nhưng chỉ khi đã có giá trị hoàn chỉnh
    return num.toFixed(1);
  }
  // Format số thành string với 1 chữ số thập phân
  return value.toFixed(1);
};

// ✅ Helper để parse điểm số từ input (hỗ trợ cả dấu , và .)
...
// ✅ Helper để normalize input (chỉ thay dấu , thành dấu .)
...
export default function GradeConfigPage() {
  ...
  const [selectedSemesters, setSelectedSemesters] = useState<string[]>(['1', '2']); // ✅ Chọn cả 2 học kỳ mặc định
  ...
  const { schoolYears: allSchoolYears, currentYearData, currentYear } = useSchoolYears();
  const { subjects } = useSubjects();
  useEffect(() => {
    const years = allSchoolYears.map(y => ({ code: y.code, name: y.name }));
    if (years.length > 0) {
      setSchoolYears(years);
      // ✅ Ưu tiên hiển thị năm học mặc định (năm học active)
      const defaultYearCode = currentYearData?.code || currentYear || years[years.length - 1].code;
      setSchoolYear(defaultYearCode);
    } else {
      const now = new Date();
    ...
    try {
      ...
      if (res && res.weights && res.rounding) {
        setConfig({
          ...
          semester: res.semester || semesterToLoad,
        });
        ...
        setConfig({
          ...
          semester: res.data.semester || semesterToLoad,
        });
      } else {
      ...
      setConfig({
        ...
        requiredSubjects: [],
        schoolYear,
        semester: selectedSemesters[0] || '1',
      });
    } finally {
  ...
  useEffect(() => {
    if (schoolYear && selectedSemesters.length > 0) {
      fetchConfig();
    }
  }, [schoolYear, selectedSemesters]);
  ...
  // Cập nhật cấu hình cho cả 2 học kỳ
  const handleSave = async () => {
    if (!schoolYear || selectedSemesters.length === 0) {
      toast({
        title: 'Lỗi',
        description: 'Vui lòng chọn năm học và ít nhất một học kỳ',
        variant: 'destructive',
      });
      return;
    }
  ...
  const getDefaultRequiredSubjects = (): RequiredSubject[] => {
    ...
    if (mathId) {
      result.push({
        ...
        classificationType: DEFAULT_REQUIRED_SUBJECTS[0].classificationType || 'excellent',
      });
    }
    ...
    if (litId) {
      result.push({
        ...
        classificationType: DEFAULT_REQUIRED_SUBJECTS[1].classificationType || 'excellent',
      });
    }
  ...
  const addRequiredSubject = (groupId?: string, classificationType: 'excellent' | 'good' | 'average' | 'weak' = 'excellent') => {
  ...
  const addSubjectToGroup = (groupId: string, classificationType?: 'excellent' | 'good' | 'average' | 'weak') => {
    // Nếu không có classificationType, lấy từ item đầu tiên trong nhóm
    if (!classificationType) {
      const groupItems = (config.requiredSubjects || []).filter(item => item.groupId === groupId);
      classificationType = groupItems[0]?.classificationType || 'excellent';
    }
    addRequiredSubject(groupId, classificationType);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Cấu hình trọng số, xếp loại học tập và môn bắt buộc cho năm học {schoolYear || '...'} - {selectedSemesters.length > 0 ? `Học kỳ ${selectedSemesters.map(s => s).join(', ')}` : 'Chưa chọn học kỳ'}
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label>Học kỳ (có thể chọn cả 2)</Label>
              <div className="flex gap-4 mt-2">
                <div className="flex items-center space-x-2">
                  <Checkbox
                    ...
                    disabled={loading || loadingConfig}
                  />
                  <Label htmlFor="semester1" className="cursor-pointer">Học kỳ 1</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox
                    id="semester2"
                    ...
                    disabled={loading || loadingConfig}
                  />
                  <Label htmlFor="semester2" className="cursor-pointer">Học kỳ 2</Label>
                </div>
              </div>
              {selectedSemesters.length === 0 && (
                <p className="text-sm text-destructive mt-1">Vui lòng chọn ít nhất một học kỳ</p>
              )}
            </div>
          </div>
        </CardContent>
            ...
            <>
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-green-50 dark:bg-green-950">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.excellent.minAverage !== undefined ? String(config.classification.excellent.minAverage) : String(DEFAULT_CLASSIFICATION.excellent.minAverage)}
                        disabled={loading}
                        onChange={(e) => {
                    ...
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.excellent.minSubjectScore !== undefined ? String(config.classification.excellent.minSubjectScore) : String(DEFAULT_CLASSIFICATION.excellent.minSubjectScore)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-blue-50 dark:bg-blue-950">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.good.minAverage !== undefined ? String(config.classification.good.minAverage) : String(DEFAULT_CLASSIFICATION.good.minAverage)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
                    ...
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.good.minSubjectScore !== undefined ? String(config.classification.good.minSubjectScore) : String(DEFAULT_CLASSIFICATION.good.minSubjectScore)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-yellow-50 dark:bg-yellow-950">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.average.minAverage !== undefined ? String(config.classification.average.minAverage) : String(DEFAULT_CLASSIFICATION.average.minAverage)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
                    ...
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.average.minSubjectScore !== undefined ? String(config.classification.average.minSubjectScore) : String(DEFAULT_CLASSIFICATION.average.minSubjectScore)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
                ...
                <div className="space-y-4 p-4 border rounded-lg bg-red-50 dark:bg-red-950">
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.weak.maxAverage !== undefined ? String(config.classification.weak.maxAverage) : String(DEFAULT_CLASSIFICATION.weak.maxAverage)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
                    ...
                    <div>
                      ...
                      <Input
                        type="text"
                        value={config.classification?.weak.maxSubjectScore !== undefined ? String(config.classification.weak.maxSubjectScore) : String(DEFAULT_CLASSIFICATION.weak.maxSubjectScore || 3.5)}
                        disabled={loading}
                        onChange={(e) => {
                          const normalized = normalizeInput(e.target.value);
                          if (normalized === '' || normalized === '.') {
              ...
              <Alert>
                ...
                <AlertDescription className="space-y-1">
                  <p>• <strong>Giỏi:</strong> Điểm TB năm ≥ {formatScore(config.classification?.excellent.minAverage || DEFAULT_CLASSIFICATION.excellent.minAverage)} và tất cả môn ≥ {formatScore(config.classification?.excellent.minSubjectScore || DEFAULT_CLASSIFICATION.excellent.minSubjectScore)}</p>
                  <p>• <strong>Khá:</strong> Điểm TB năm ≥ {formatScore(config.classification?.good.minAverage || DEFAULT_CLASSIFICATION.good.minAverage)} và tất cả môn ≥ {formatScore(config.classification?.good.minSubjectScore || DEFAULT_CLASSIFICATION.good.minSubjectScore)}</p>
                  <p>• <strong>Trung bình:</strong> Điểm TB năm ≥ {formatScore(config.classification?.average.minAverage || DEFAULT_CLASSIFICATION.average.minAverage)} và tất cả môn &gt; {formatScore(config.classification?.average.minSubjectScore || DEFAULT_CLASSIFICATION.average.minSubjectScore)}</p>
                  <p>• <strong>Yếu:</strong> Điểm TB năm &lt; {formatScore(config.classification?.weak.maxAverage || DEFAULT_CLASSIFICATION.weak.maxAverage)} hoặc có môn &lt; {formatScore(config.classification?.weak.maxSubjectScore || DEFAULT_CLASSIFICATION.weak.maxSubjectScore || 3.5)}</p>
                </AlertDescription>
              </Alert>
            </>
          )}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <Select
                ...
                onValueChange={(value) => {
                  if (value) {
                    addRequiredSubject(undefined, value as 'excellent' | 'good' | 'average' | 'weak');
                  }
                }}
                disabled={loading}
              >
                <SelectTrigger className="w-40">
                  <SelectValue placeholder="Thêm môn cho loại..." />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="excellent">Thêm môn - Loại Giỏi</SelectItem>
                  <SelectItem value="good">Thêm môn - Loại Khá</SelectItem>
                  <SelectItem value="average">Thêm môn - Loại Trung bình</SelectItem>
                  <SelectItem value="weak">Thêm môn - Loại Yếu</SelectItem>
                </SelectContent>
              </Select>
            </div>
          </div>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-4 p-4 bg-muted/50 rounded-lg">
            <div className="flex items-center gap-2 flex-1">
              ...
              <Input
                type="text"
                value={defaultMinScore !== undefined ? String(defaultMinScore) : ''}
                onChange={(e) => {
                  const normalized = normalizeInput(e.target.value);
                  if (normalized === '' || normalized === '.') {
                    setDefaultMinScore(0);
                  } else {
                    const parsed = parseFloat(normalized);
                    if (!isNaN(parsed)) {
                      setDefaultMinScore(parsed);
                    }
                  }
                }}
                onBlur={(e) => {
          ...
          {loadingConfig ? (
            ...
            <>
                  ...
                  <div key={groupId} className="p-4 border rounded-lg space-y-3">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <Select
                          value={items[0]?.classificationType || 'excellent'}
                          onValueChange={(v) => {
                            items.forEach((item) => {
                              updateRequiredSubject(item.originalIndex, 'classificationType', v);
                            });
                          }}
                          disabled={loading}
                        >
                          ...
                          <SelectContent>
                            <SelectItem value="excellent">Loại Giỏi</SelectItem>
                            <SelectItem value="good">Loại Khá</SelectItem>
                            <SelectItem value="average">Loại Trung bình</SelectItem>
                            <SelectItem value="weak">Loại Yếu</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      {items.length > 1 && (
                    ...
                    <div className="space-y-2">
                        ...
                        return (
                          <div key={originalIndex} className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <Input
                                type="text"
                                value={item.minScore !== undefined ? String(item.minScore) : ''}
                                onChange={(e) => {
                                  const normalized = normalizeInput(e.target.value);
                                  if (normalized === '' || normalized === '.') {
                                    updateRequiredSubject(originalIndex, 'minScore', 0);
                                  } else {
                                    const parsed = parseFloat(normalized);
                                    if (!isNaN(parsed)) {
                                      updateRequiredSubject(originalIndex, 'minScore', parsed);
                                    }
                                  }
                                }}
                                onBlur={(e) => {
                      ...
                      {items.length > 1 && (
                        <div className="text-xs text-muted-foreground mt-2">
                          <span className="font-semibold">Loại xếp loại: </span>
                          {items[0]?.classificationType === 'excellent' ? 'Giỏi' :
                           items[0]?.classificationType === 'good' ? 'Khá' :
                           items[0]?.classificationType === 'average' ? 'Trung bình' :
                           items[0]?.classificationType === 'weak' ? 'Yếu' : 'Chưa xác định'}
                          {' | '}
                          {items[0].requireAll
              ...
              <Alert>
                ...
                <AlertDescription>
                  <p className="mb-2">
                    • Môn bắt buộc được sử dụng để kiểm tra điều kiện xếp loại học lực theo từng loại (Giỏi, Khá, Trung bình, Yếu).
                  </p>
                  <p className="mb-2">
                    • Mỗi nhóm môn có thể được gán cho một loại xếp loại cụ thể (Giỏi, Khá, Trung bình, Yếu).
                  </p>
                  <p className="mb-2">
                  ...
                  <p className="mt-2">
                    <strong>Ví dụ:</strong> Nhóm Toán và Văn, loại "Giỏi", yêu cầu "1 trong 2", điểm tối thiểu {formatScore(defaultMinScore)} → Để xếp loại Giỏi, học sinh chỉ cần Toán ≥ {formatScore(defaultMinScore)} HOẶC Văn ≥ {formatScore(defaultMinScore)}.
                  </p>
                </AlertDescription>
              </Alert>
            </>
