...
import { Table, Button, Modal, Form, Input, DatePicker, TimePicker, InputNumber, message, Space, Spin, Popconfirm, Select } from "antd";
...
export default function ExamSchedules({ examId }: { examId: string }) {
  ...
  const [rooms, setRooms] = useState<{_id:string,code:string}[]>([]);
  ...
  const fetchSubjectsAndRooms = async () => {
    try {
      const [subjRes, roomRes] = await Promise.all([
        subjectApi.getSubjects(),
        api.getExamRooms(examId)
      ]);
      setSubjects(subjRes);
      setRooms(roomRes.data);
    } catch(err:any){
      message.error(err?.message || "Lỗi khi tải môn/ phòng");
    }
  };
  ...
  const columns = [
    { 
      title: "Môn", 
      dataIndex: "subjectId",
      render: (id: string) => subjects.find(s => s._id === id)?.name || id
    },
    { 
      title: "Ngày", 
      dataIndex: "date", 
    ...
    { 
      title: "Phòng", 
      dataIndex: "roomId",
      render: (id: string) => rooms.find(r => r._id === id)?.code || id
    },
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="roomId" label="Phòng" rules={[{ required: true }]}>
            ...
            >
              {rooms.map(r => <Option key={r._id} value={r._id}>{r.code}</Option>)}
            </Select>
          </Form.Item>
