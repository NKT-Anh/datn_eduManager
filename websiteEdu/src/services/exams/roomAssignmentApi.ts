...
export const roomAssignmentApi = {
  ...
  async autoAssign(scheduleId: string, data?: any) {
    const res = await api.post(`/exam/room-assignments/auto/${scheduleId}`, data);
    return res.data;
  },

  /** 📋 Lấy danh sách học sinh đã xếp chỗ theo lịch thi */
  async getBySchedule(scheduleId: string) {
    const res = await api.get(`/exam/room-assignments/${scheduleId}`);
    return res.data;
  },

  /** ✏️ Cập nhật chỗ ngồi */
  async update(id: string, data: Partial<RoomAssignment>) {
    const res = await api.put(`/exam/room-assignments/${id}`, data);
    return res.data;
  },

  /** 🗑️ Reset toàn bộ danh sách xếp phòng cho 1 lịch thi */
  async reset(scheduleId: string) {
    const res = await api.delete(`/exam/room-assignments/reset/${scheduleId}`);
    return res.data;
  },

  /** 🧾 Xuất danh sách PDF */
  async exportPdf(scheduleId: string) {
    const res = await api.get(`/exam/room-assignments/export/${scheduleId}/pdf`, {
      responseType: "blob",
    });
    return res.data;
  },
  ...
  async manualAssign(roomId: string, studentIds: string[]) {
    const res = await api.post("/exam/room-assignments/manual", {
      roomId,
      studentIds,
    });
    return res.data;
